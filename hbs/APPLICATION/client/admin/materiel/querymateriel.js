HBSConvertHelper.init(function() {
	...
	querygrid.getView().on("refresh", function(view) {
		...
		var updateBtnFun = function() {
			// 要访问的 url 地址
			var url = ["/materiel/editormateriel.jsp?editorType=update&partNo=", encodeString(this.config.get("partNo"))].join("");
			// 打开指定页面
		...
		var deleteBtnFun = function() {
			Ext.Msg.confirm("提示", "您要执行的是删除操作，请确认是否继续？", function(btn) {
				...
				ExtConvertHelper.request("/partNo/partNo!del.action?partNo.partNo=" + encodeString(this.config.get("partNo")), null, ExtConvertHelper.defaultDeleteFun);
			}, this);
		}
		...
		for(var i = 0 ; i < view.ds.getCount() ; i++) {
			...
			// 将需要的链接渲染到此列
			HBSConvertHelper.renderATag2Cell(cpartNo_cell.innerText, "/materiel/detailmateriel.jsp?pageType=query&partNo="+ encodeString(view.ds.getAt(i).get("partNo")), "open", cpartNo_cell);
