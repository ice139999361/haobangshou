HBSConvertHelper.init(function() {
	...
	(function() {
		addBtn.on("click", function() {
			// 要访问的 url 地址
			var url = "/invoice/editorcustomerinvoice.jsp";
			// 打开指定页面
		...
		querygrid.getView().on("refresh", function(view) {
			...
			var updateBtnFun = function() {
				// 要访问的 url 地址
				var url = ["/invoice/editorcustomerinvoiced.jsp?editorType=update&invoice.invoiceSeqId=", this.config.get("invoiceSeqId")].join("");
				// 打开指定页面
				HBSConvertHelper.openNewWin(url);
			};
			...
			for(var i = 0 ; i < view.ds.getCount() ; i++) {
				...
				// 将需要的链接渲染到此列
				HBSConvertHelper.renderATag2Cell(commCode_cell.innerText, "/invoice/detailcustomerinvoiced.jsp?pageType=query&invoice.invoiceSeqId=" + record.get("invoiceSeqId"), "open", commCode_cell);
