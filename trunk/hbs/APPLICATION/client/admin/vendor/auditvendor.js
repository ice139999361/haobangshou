...
// 查询数据的地址
var querygridUrl = "/vendorInfo/vendorInfoMgr!listForAudit.action";
		...
		for(var i = 0 ; i < view.ds.getCount() ; i++) {
		  ...
		  // 创建按钮到操作列
			...
			updateBtn.on("click", function() {
				// 要访问的 url 地址
				var url = ["/vendor/detailvendor.jsp?pageType=audit&editorType=update&baseSeqId=", this.config.get("baseSeqId"), "&state=", this.config.get("state")].join("");
				// 打开指定页面
