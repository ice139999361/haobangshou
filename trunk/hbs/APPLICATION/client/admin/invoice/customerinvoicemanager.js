	...
	(function() {
		...
		querygrid.getView().on("refresh", function(view) {
			...
			var updateBtnFun = function() {
				// 要访问的 url 地址
				var url = ["/invoice/editorcustomerinvoiced.jsp?editorType=update&commCode=", this.config.get("commCode")
			...
			for(var i = 0 ; i < view.ds.getCount() ; i++) {
				...
				// 将需要的链接渲染到此列
				HBSConvertHelper.renderATag2Cell(commCode_cell.innerText, "/invoice/detailcustomerinvoiced.jsp?pageType=query&baseSeqId=" + record.get("baseSeqId"), "open", commCode_cell);
